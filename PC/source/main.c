		...
		switch(buffer.command) {
			...
			case KEYS:
				...
				if((currentKeys & KEY_TOUCH)) {
					if(keyboardActive) {
						...
						if(settings.mouseSpeed) {
							...
							// Map touchscreen Y-coordinate to the bottom half of the screen
							int bottomHalfY = (int)((double)currentTouch.y * (heightMultiplier / 2.0)) + (screenHeight / 2);
							SetCursorPos((int)((double)currentTouch.x * widthMultiplier), bottomHalfY);
						}
					}
					...
					else if(settings.touch == regions && !holdTouch) {
						if (lastTouch.x > 160){
							if (lastTouch.y > 120){
								pressKey(settings.RegionRB);
							} else {
								pressKey(settings.RegionRT);
							}
						} else {
							if (lastTouch.y > 120){
								pressKey(settings.RegionLB);
							} else {
								pressKey(settings.RegionLT);
							}
						}
						holdTouch = true;
					}
					else if(settings.touch == tap && !holdTouch) {
						pressKey(settings.Tap);
						holdTouch = true;
					}
					...
					if(settings.touch == regions && holdTouch){
						releaseKey(settings.RegionRB);
						releaseKey(settings.RegionRT);
						releaseKey(settings.RegionLB);
						releaseKey(settings.RegionLT);
					}

					if(settings.touch == tap && holdTouch){
						releaseKey(settings.Tap);
					}
				...
				if(settings.circlePad == mouse) {
					...
					//printf("\rCirclePad: %d, %d\t\t", circlePad.x, circlePad.y);
					joyX = clamp((applyDeadZoneX(circlePad.x, circlePad.y, settings.deadZone1) + 128) * 128, 0, 32767);
					joyY = clamp((128 - applyDeadZoneY(circlePad.x, circlePad.y, settings.deadZone1)) * 128, 0, 32767);
				}

				else if(settings.circlePad == joystick2) {
					joyRX = clamp((applyDeadZoneX(circlePad.x, circlePad.y, settings.deadZone2) + 128) * 128, 0, 32767);
					joyRY = clamp((128 - applyDeadZoneX(circlePad.x, circlePad.y, settings.deadZone2)) * 128, 0, 32767);
				}
				...
				if(settings.cStick == mouse) {
					...
					joyX = clamp((applyDeadZoneX(cStick.x, cStick.y, settings.deadZone1) + 128) * 128, 0, 32767);
					joyY = clamp((128 - applyDeadZoneY(cStick.x, cStick.y, settings.deadZone1)) * 128, 0, 32767);
				}

				else if(settings.cStick == joystick2) {
					joyRX = clamp((applyDeadZoneX(cStick.x, cStick.y, settings.deadZone2) + 128) * 128, 0, 32767);
					joyRY = clamp((128 - applyDeadZoneY(cStick.x, cStick.y, settings.deadZone2)) * 128, 0, 32767);
				}
